...
def parse_interval(s: str) -> int:
    ...
    if not m:
        raise ValueError("interval must look like 5m, 10m, 1h, 6h, 1d")
    n = int(m.group(1))
    unit = m.group(2)
    mult = {"m": 60, "h": 3600, "d": 86400}[unit]
    sec = n * mult
    if sec < MIN_SECONDS or sec > MAX_SECONDS:
        raise ValueError("interval must be between 5m and 1d")
    return sec


def main() -> int:
    parser = argparse.ArgumentParser(description="Install/update OpenClaw cron job for twitter collector")
    parser.add_argument("--interval", default="1h", help="5m to 1d, e.g. 5m, 10m, 1h, 1d")
